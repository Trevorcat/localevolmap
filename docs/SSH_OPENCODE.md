...
```bash
REMOTE_HOST=<server-ip> REMOTE_USER=ubuntu APP_DIR=/opt/local-evomap \
  bash deployment/scripts/opencode-remote-command.sh "bash deployment/health-check.sh"
```
...
`deployment/scripts/opencode-remote-command.sh` 会在远端自动：
...
ENV_FILE=/opt/local-evomap/.env.staging REMOTE_HOST=<server-ip> \
  bash deployment/scripts/opencode-remote-command.sh "node -v"
```
