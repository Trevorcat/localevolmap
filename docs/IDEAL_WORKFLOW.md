...
> **⚠️ v2026.03.14 架构变更**：进化引擎已切换为**纯算法模式**。以下流程图中所有标注"LLM 生成"、"LLM 合成"的步骤，现已由纯算法策略引导（`guidance` 输出）和算法蒸馏（覆盖缺口分析、策略漂移分裂、高频基因提炼）替代。流程图保留 LLM 标注作为历史参考，实际运行时不再调用外部 LLM。主要变化：
> - **阶段五**：不再调用 LLM，改为生成结构化 `guidance` 引导文本，`changes[]` 始终为空
> - **阶段七**：无重试逻辑（无 LLM 生成错误），评分基于算法 `confidence` 计算
> - **阶段九**：蒸馏由 `synthesizeGeneAlgorithmic()` 算法完成，无需 LLM
...
| **胶囊负反馈** | 失败胶囊仍可被选中 ❌ | 预过滤排除 failed / _deleted 胶囊 ✅ |
